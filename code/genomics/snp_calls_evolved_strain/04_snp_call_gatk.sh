#!/bin/bash
#this bash script passes cleaned short read sequences through the gatk pipeline to identify introduced mutations
...
#this script is parallel to the breseq pipeline
#some steps may require node time

########################################################################################################v
#Reference Genome Preparation 
#note: You only need to run this once per reference genome. If  
#allorhizobium_anc_assembly.fasta is already indexed, you can comment this section out
########################################################################################################v
module load StdEnv/2023
module load bwa/0.7.18
ancestor_path="path/to/ancestor_assembly/allorhizobium_anc_assembly.fasta"
...
java -jar /cvmfs/soft.computecanada.ca/easybuild/software/2023/x86-64-v3/Core/gatk/4.6.1.0/gatk-package-4.6.1.0-local.jar GenotypeGVCFs -R /home/kricks/scratch/saline_evolution_update/ancestor_genome/nf9_ancestral.fasta -V ref.vcf -ploidy 1 -O new.vcf -stand-call-conf 30 > genotype_gVCFs.out 
java -jar /cvmfs/soft.computecanada.ca/easybuild/software/2023/x86-64-v3/Core/gatk/4.6.1.0/gatk-package-4.6.1.0-local.jar VariantsToTable -V new.vcf -R /home/kricks/scratch/saline_evolution_update/ancestor_genome/nf9_ancestral.fasta -F CHROM -F POS -F REF -F ALT -F QUAL -F AF -F ANN -F DP -GF GT -O gatk_snps.table
